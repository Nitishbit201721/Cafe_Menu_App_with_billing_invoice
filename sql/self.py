...
class CafeApp:
    def __init__(self, root):
        ...
        self.order = {}
        self.dark_mode = tk.BooleanVar(value=False)  # Initialize dark mode to False
        self.search_var = tk.StringVar()
        self.search_var.trace("w", self.dynamic_search)  # Dynamic search on text change

        # Café Logo/Title
        self.logo_label = ttk.Label(root, text="Café Delight", font=("Georgia", 24, "bold"), foreground="#964409")
        ...
        self.search_entry.bind("<Return>", lambda event: self.search_item())  # Enter key triggers search
        ...
        self.qty_entry = ttk.Entry(self.qty_frame, textvariable=self.qty_var, width=5)
        self.qty_entry.pack(side=tk.LEFT)
        self.qty_entry.bind("<KeyRelease>", self.validate_qty)  # Real-time quantity validation
    ...
    def set_theme(self):
        ...
        light_bg = "#F8F8F8"
        light_text = "#08C664"
        light_category_text = "#005b96"
        light_button_bg = "#005b96"
        light_button_fg = "white"
        light_menu_item_bg = "#E0E0E0"
        light_menu_item_fg = "#3E059A"
        ...
        style.map("TButton", background=[("active", "#003f63")])
        style.configure("MenuItem.TButton", background=light_menu_item_bg, foreground=light_menu_item_fg, font=("Segoe UI", 10))
        style.map("MenuItem.TButton", background=[("active", "#C0C0C0")])

        self.logo_label.config(background=light_bg, foreground="#8B4513")
        self.current_order_text.config(bg="white", fg="black", insertbackground="black")
        self.search_entry.config(background="white", foreground="black")
        self.qty_entry.config(background="white", foreground="black")
    ...
    def toggle_dark_mode(self):
        ...
        if self.dark_mode.get():
            dark_bg = "#BE1616"
            dark_text = "#FFFFFF"
            dark_category_text = "#6495ED"
            dark_button_bg = "#4682B4"
            dark_button_fg = "white"
            dark_menu_item_bg = "#444444"
            ...
            style.map("TButton", background=[("active", "#36709E")])
            style.configure("MenuItem.TButton", background=dark_menu_item_bg, foreground=dark_menu_item_fg)
            style.map("MenuItem.TButton", background=[("active", "#555555")])

            self.logo_label.config(background=dark_bg, foreground="#D2B48C")
            self.current_order_text.config(bg="#444", fg="white", insertbackground="white")
            self.search_entry.config(background="#444", foreground="white")
            self.qty_entry.config(background="#444", foreground="white")
        else:
            self.set_theme()

    def validate_qty(self, *args):
        qty = self.qty_var.get()
        if qty and not qty.isdigit():
            self.qty_var.set("1")  # Reset to default if invalid
        ...
        for category, items in categories.items():
            ...
            for item in items:
                item_button = ttk.Button(self.scrollable_frame,
                                         text=f"{item:<30}: ${menu[item]:.2f}",
                                         command=lambda i=item: self.add_item_from_menu_click(i),
                                         style="MenuItem.TButton")
                item_button.pack(anchor="w", padx=10, fill=tk.X, pady=2)
                item_button.bind("<Enter>", lambda e, b=item_button: b.config(cursor="hand2"))  # Hand cursor on hover

    def dynamic_search(self, *args):
        self.search_item()  # Update menu as user types
                ...
                if items_in_cat:
                    ...
                    for item in items_in_cat:
                        item_button = ttk.Button(self.scrollable_frame,
                                                 ...
                                                 style="MenuItem.TButton")
                        item_button.pack(anchor="w", padx=10, fill=tk.X, pady=2)
                        item_button.bind("<Enter>", lambda e, b=item_button: b.config(cursor="hand2"))
                    displayed_any = True
    ...
    def add_item_from_menu_click(self, item_name):
        try:
            ...
            messagebox.showerror("Error", str(e) if str(e) != "" else "Invalid quantity. Please enter a positive number.")
            return
    ...
    def update_order_display(self):
        ...
        if not self.order:
            ...
            order_summary = f"{'Item':<25}{'Qty':<5}{'Price':<8}{'Total'}\n"
            order_summary += "-" * 50 + "\n"
            for item, qty in self.order.items():
                order_summary += f"{item:<25}{qty:<5}${menu[item]:<8.2f}${menu[item] * qty:.2f}\n"
            order_summary += "-" * 50
    ...
    def generate_invoice(self):
        ...
        invoice_win.resizable(True, True)  # Allow invoice window resizing
        if self.dark_mode.get():
    ...
    def save_invoice(self, text):
        ...
        try:
            ...
            messagebox.showerror("Error", "Permission denied. Please check file permissions or save location.")
